...
function FormRangePicker(
    ...
    // maxDate və minDate-i dayjs obyektinə çeviririk
    ...
    // Tarixləri disabled etmək üçün funksiya
    ...
    const generateDateRangeItems = useCallback(() => {
        ...
        const isDateInRange = (date) => {
            if (convertedMaxDate && date.isAfter(convertedMaxDate, 'day')) return false;
            if (convertedMinDate && date.isBefore(convertedMinDate, 'day')) return false;
            return true;
        };

        if (today) {
            const todayDate = dayjs();
            if (isDateInRange(todayDate)) {
                items.push({
        ...
        if (yesterday) {
            ...
            if (isDateInRange(yesterdayDate)) {
                items.push({
        ...
        if (last7days) {
            ...
            if (isDateInRange(last7Start) && isDateInRange(last7End)) {
                items.push({
                    key: RANGE_PRESETS.LAST_7_DAYS,
                    order: 2,
                    value: [last7Start, last7End],
        ...
        if (last30days) {
            ...
            if (isDateInRange(last30Start) && isDateInRange(last30End)) {
                items.push({
                    key: RANGE_PRESETS.LAST_30_DAYS,
                    order: 3,
                    value: [last30Start, last30End],
        ...
        if (last90Days) {
            const last90Start = dayjs().subtract(89, 'day');
            const last90End = dayjs();
            if (isDateInRange(last90Start) && isDateInRange(last90End)) {
                items.push({
                    key: RANGE_PRESETS.LAST_90_DAYS,
                    order: 4,
                    value: [last90Start, last90End],
        ...
        if (thisMonth) {
            ...
            if (isDateInRange(thisMonthStart) && isDateInRange(thisMonthEnd)) {
                items.push({
        ...
        if (lastMonth) {
            ...
            if (isDateInRange(lastMonthStart) && isDateInRange(lastMonthEnd)) {
                items.push({
        ...
        if (all) {
            const allStart = allStartDate ? dayjs(allStartDate) : dayjs().subtract(2, 'year').startOf('year');
            const allEnd = dayjs();
            if (isDateInRange(allStart) && isDateInRange(allEnd)) {
                items.push({
                    key: RANGE_PRESETS.ALL,
                    order: 100,
                    value: [allStart, allEnd],
        ...
        return items;
    }, [today, yesterday, last7days, last30days, last90Days, thisMonth, lastMonth, all, allStartDate, convertedMaxDate, convertedMinDate, translate]);
